...
def main():
    ...
    if training_args.do_train:
        ...
        # train_dataset = raw_datasets["test"].select(range(3500, 5150))     # ThangPM: HOT FIXED for training on PSD
    ...
    if training_args.do_eval:
        ...
        # eval_examples = raw_datasets["test"].select(range(3000, 3500))  # ThangPM: HOT FIXED  for training on PSD
    ...
    if training_args.do_predict:
        ...
        # predict_examples = raw_datasets["test"].select(range(0, 3000))  # ThangPM: HOT FIXED  for training on PSD
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        if data_args.version_2_with_negative:
            ...
            if "phrase_sense_disambiguation" not in data_args.dataset_name.lower():
                ...
                # ThangPM: Adding answer_start
                formatted_predictions = [{"id": k, "prediction_text": v, "answer_start": predictions_with_indices[k]["offsets"][0]} for k, v in predictions.items()]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # ThangPM: Compute EM and F1 taking predicted answer_start into consideration for PSD
        formatted_predictions = [{"id": pred['id'], "prediction_text": pred['prediction_text']} for pred in p.predictions]
        ...
        for pred, label in zip(p.predictions, p.label_ids):
            ...
            if pred['prediction_text'].lower() == answer_text.lower():
                ...
                if pred['answer_start'] == answer_start:
                    ...
                    print({
                        "id": pred["id"],
                        "pred_text": [pred["prediction_text"], pred["answer_start"]],
                        "answer_text": [answer_text, answer_start],
                    })

            f1_count += compute_f1(answer_text, pred['prediction_text'])

            pred_range = range(pred['answer_start'], pred['answer_start'] + len(pred['prediction_text']))
            answer_range = range(answer_start, answer_start + len(answer_text))
            if len(set(pred_range).intersection(set(answer_range))) > 0:
                f1_with_index_count += compute_f1(answer_text, pred['prediction_text'])

        em_score = round(em_count / len(p.label_ids), 4) * 100
        f1_score = round(f1_count / len(p.label_ids), 4) * 100

        em_with_index_score = round(em_with_index_count / len(p.label_ids), 4) * 100
        f1_with_index_score = round(f1_with_index_count / len(p.label_ids), 4) * 100

        print("Computed results: {}".format({"exact_match": em_score, "exact_match_with_index": em_with_index_score,
                                             "f1": f1_score, "f1_with_index": f1_with_index_score}))
        print("Reference results: {}".format(metric.compute(predictions=formatted_predictions, references=p.label_ids)))
