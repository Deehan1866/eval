...
class ProbingModel(LightningModule):
    def __init__(self, input_dim=1536, hidden_dim=115, train_dataset=None, valid_dataset=None, test_dataset=None):
        ...
        # Linear layers
        self.linear = nn.Linear(hidden_dim * 1, 256)  # doubled hidden_dim because of bidirectional
        self.linear2 = nn.Linear(256, 1)
        self.output = nn.Sigmoid()

        # Dropout layer
        self.dropout = nn.Dropout(p=0.5)  # Adjust dropout rate as needed
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1, self.input_dim)  # Reshape x
